...
struct Point
{
	...
	//0,p1p垂直p1p2；1,p2p垂直p1p2；(0,1),p在p1点垂线与p2点垂线之间
	...
	bool inRect(Point& p1, Point& p2){
		return LEQ((p1.x - x)*(p2.x - x), 0) && LEQ((p1.y - y)*(p2.y - y), 0);
	}
...
double Point::lineSegArrayDis(Point* p, int lineNum, Point& ret)
{
	...
	for (int i = 0; i < lineNum; i++){
		...
		if (LE(td, mind)){ mind = td; ret = tp; }
	}
	return mind;
}

//donnot include extream points, and donnot include coincidence.
inline bool lineSegLineSegIntersect(Point& p1, Point& p2, Point& q1, Point& q2){
...
inline bool lineSegLineSegIntersect2(Point& p1, Point& p2, Point& q1, Point& q2){
	if (!LEQ(min(q1.x, q2.x), max(p1.x, p2.x)) || !LEQ(min(p1.x, p2.x), max(q1.x, q2.x))
		|| !LEQ(min(q1.y, q2.y), max(p1.y, p2.y)) || !LEQ(min(p1.y, p2.y), max(q1.y, q2.y)))
		return false;
	Point pq1 = p1 - q1, p12 = p2 - p1, q12 = q2 - q1;
	return SGN(pq1*q12)*SGN((p2 - q1)*q12) <= 0 && SGN(pq1*p12)*SGN((p1 - q2)*p12) <= 0;
}
...
inline Point lineLineIntersectPoint(Point& p1, Point& p2, Point& q1, Point& q2){
	Point q12 = q2 - q1;
	double k = (p2 - p1)*q12;
...
int lineConvexIntersectPointInit(Point* p, int n, double angle[])
{
	...
	for (int i = 0, j = n - 1; i < n; j = i++)
		angle[j] = p[i].angle(p[j]);
	do angle[ret + n] = angle[ret++];
	while (LE(angle[ret - 1], angle[ret]) && ret < n);
	return ret;
}
//ret和ret2分别为距直线最近和最远的点下标
int lineConvexIntersect(Point& p1, Point& p2, Point* p, int n, double angle[], int offset, int &ret1, int &ret2)
{
	int pos[2];
	double k[2];
	k[0] = p1.angle(p2); k[1] = k[0] <= 0 ? k[0] + PI : k[0] - PI;
	for (int i = 0; i < 2; i++){
		pos[i] = (upper_bound(angle + offset, angle + offset + n, k[i] - EPS) - angle) % n;
		if (p[pos[i]].onLine(p1, p2))
			return p[NEXT(pos[i], n)].onLine(p1, p2) ? 3 : 1;
	}
	ret1 = pos[0]; ret2 = pos[1];
	return p[pos[0]].direction(p1, p2) == p[pos[1]].direction(p1, p2) ? 0 : 2;
}
void lineConvexIntersectPoint(Point& p1, Point& p2, Point* p, int n, int i1, int i2, Point &ret1, Point &ret2)
{
	for (int i = 0, l, r; i < 2; i++){
		if (i){ l = min(i1, i2); r = max(i1, i2); }
		else{ l = max(i1, i2); r = min(i1, i2) + n; }
		while (l < r){
			int mid = (l + r) >> 1;
			if (p[mid % n].direction(p1, p2) == p[r % n].direction(p1, p2))r = mid;
			else l = mid + 1;
		}
		l %= n;
...
bool lineSegInPolygon(Point p1, Point p2, Point* p, int n)
{
	...
	do{
		i = NEXT(i, n);
		d1 = d2; d2 = p[i].direction(p1, p2);
		if (d2*d == -1){
			if (lineSegLineSegIntersect(p[i], p[j], p1, p2))return false;
			if (d1 == 0 && p1 < p[id] && p[id] < p2 && p1 < p[j] && p[j] < p2)return false;
			d = d2;
		}
	...
	if (s1 == 0 && s2 == 0){
		...
		return ((q1 + p[id])*0.5).inPolygon(p, n) >= 0 && ((q2 + p[id])*0.5).inPolygon(p, n) >= 0;
	}
	return true;
}
...
void convexHullSorted(Point* p, int n, Point* up, int& retUp, Point* down, int& retDown)
{
	retUp = retDown = 0;
	for (int i = 0; i < n; i++){
...
int halfPlainIntersect(Point(*p)[2], int n, Point* ret)
{
	...
	for (int i = 0; i < n; i++)
		v[i] = make_pair(p[i][1].angle(p[i][0]), p[i]);
	sort(v.begin(), v.end());
	int m = 0, l = 0, r = 0;
	for (int i = 1; i < n; i++){
		if (!EQ(v[i].first, v[m].first))v[++m] = v[i];
		else if (!judge(v[i].second[0], v[m]))v[m] = v[i];
	}
	if (EQ(v[0].first + 2 * PI, v[m].first) && !judge(v[m--].second[0], v[0]))v[0] = v[m + 1];
	vector<Point> q(n);
	line[0] = v[0];
	for (int i = 1; i <= m; i++){
...
void nearestPointsInternal(Point* p, int left, int right, NearestPointsStruct& s)
{
	if (right - left < 8){
	...
	for (int i = mid - 1; i >= left && l - p[i].x < d; i--)
		s.v.push_back(p[i]);
	for (int i = mid; i<right && p[i].x - l < d; i++)
		s.v.push_back(p[i]);
	sort(s.v.begin(), s.v.end(), nearestPointsCmp);
	for (unsigned int i = 0; i < s.v.size(); i++){
		for (unsigned int j = i + 1; j < s.v.size() && s.v[j].y - s.v[i].y < d; j++){
			double td2 = s.v[j].dis2(s.v[i]);
			if (td2 < s.d2){
				s.d2 = td2;
				s.p1 = s.v[i]; s.p2 = s.v[j];
			}
		}
	}
}
...
void circleLineIntersect(Point& o, double r, Point& p1, Point& p2, Point& ret1, Point& ret2)
{
...
void circleCircleIntersectPoint(Point& o1, double r1, Point& o2, double r2, Point& ret1, Point& ret2)
{
...
void circleCirclePointcutOuter(Point& o1, double r1, Point& o2, double r2, Point* ret)
{
	Point o12 = o2 - o1;
	...
	ret[0] = foot1 + line*(t*r1); ret[1] = foot2 + line*(t*r2);
	ret[2] = foot1 * 2 - ret[0]; ret[3] = foot2 * 2 - ret[1];
}
void circleCirclePointcutInner(Point& o1, double r1, Point& o2, double r2, Point* ret)
{
	Point o12 = o2 - o1;
	double d12 = o12.r2(), r = (r1 + r2) / d12;
	Point foot1 = o1 + o12*(r*r1), foot2 = o2 - o12*(r*r2);
